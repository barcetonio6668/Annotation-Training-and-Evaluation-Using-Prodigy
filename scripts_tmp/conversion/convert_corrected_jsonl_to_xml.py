        ...
        for line_num, line in enumerate(f, 1):
            try:
                ...
                ET.SubElement(
                    meta_elem,
                    "input_hash"
                ).text = str(data.get("_input_hash", ""))

                ET.SubElement(
                    meta_elem,
                    "task_hash"
                ).text = str(data.get("_task_hash", ""))

                ET.SubElement(
                    meta_elem,
                    "timestamp"
                ).text = str(data.get("_timestamp", ""))

                ET.SubElement(
                    meta_elem,
                    "view_id"
                ).text = data.get("_view_id", "")

            except json.JSONDecodeError as e:
                print(
                    f"Error parsing line {line_num} "
                    f"in {jsonl_path}: {e}"
                )
